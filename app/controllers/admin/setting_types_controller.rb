class Admin::SettingTypesController < AdminController
  ...
  def create
    ...
    if @setting_type.save
      redirect_to admin_setting_type_path(@setting_type), :flash => { :success => 'Setting type created' }
    else
  ...
  def update
    if @setting_type.update(setting_type_params)
      redirect_to admin_setting_type_path(@setting_type), :flash => { :success => 'Setting type updated' }
    else
  ...
  def destroy
    ...
    redirect_to admin_setting_types_path
  end
  ...
  def set_setting_type
    ...
    redirect_to admin_setting_types_path, :flash => { :error => 'Unable to find setting type ' } unless @setting_type
  end
